...
const ResourceManagement = () => {
  ...
  const [resources, setResources] = useState({
    ...
    "Medical Centers": [
      {
        id: "MC-1198",
        name: "Mercy Trauma Unit",
        location: "Harbor District, West Zone",
        capacity: 72,
        maxCapacity: 120,
        status: "OPERATIONAL",
        channel: "+1 (800) MED-1198",
        color: "bg-emerald-400",
      },
      {
        id: "MC-2207",
        name: "Field Care Outpost",
        location: "East Basin, Relief Sector",
        capacity: 19,
        maxCapacity: 80,
        status: "CRITICAL FILL",
        channel: "+1 (800) MED-2207",
        color: "bg-red-500",
      },
    ],
    "Rescue Teams": [
      {
        id: "RT-3344",
        name: "Delta Search Squad",
        location: "Sector 7 Perimeter",
        capacity: 25,
        maxCapacity: 30,
        status: "DEPLOYED",
        channel: "+1 (800) RES-3344",
        color: "bg-sky-400",
      },
      {
        id: "RT-8821",
        name: "Rapid Water Response",
        location: "North Dam Channel",
        capacity: 12,
        maxCapacity: 20,
        status: "STAGING",
        channel: "+1 (800) RES-8821",
        color: "bg-amber-400",
      },
    ],
  });
      ...
      try {
        ...
        (data.resources || []).forEach((resource) => {
          ...
          grouped[tabKey].push({
            ...
            name: resource.name,
            location:
              resource.type_label || resource.location || "Unknown location",
            capacity: parseInt(resource.capacity_text?.split("/")[0]) || 0,
            maxCapacity: parseInt(resource.capacity_text?.split("/")[1]) || 0,
            status: resource.status,
            channel: resource.distance_label || resource.capacity_text || "N/A",
            color: resource.status_color_class || "bg-blue-400",
            category: tabKey,
          });
        });

        if (!cancelled) {
          setResources((prev) => ({ ...prev, ...grouped }));
        }
      } catch (error) {
  ...
  const getCategoryKey = (tab) => {
    if (tab === "Medical Centers") return "medical";
    if (tab === "Rescue Teams") return "rescue";
    return "shelter";
  };

  const normalizeStatusColor = (status) => {
    if (status === "CRITICAL FILL") return "bg-red-500";
    if (status === "DEPLOYED") return "bg-sky-400";
    if (status === "OPERATIONAL") return "bg-emerald-400";
    if (status === "STAGING") return "bg-blue-400";
    return "bg-slate-400";
  };

  const handleAddResource = async (e) => {
    e.preventDefault();
    const categoryKey = getCategoryKey(activeTab);
    const payload = {
      category: categoryKey,
      type_label: formValues.location || formValues.name,
      name: formValues.name,
      status: formValues.status,
      status_color_class: normalizeStatusColor(formValues.status),
      distance_label: formValues.channel,
      capacity_text: `${formValues.capacity || 0}/${formValues.maxCapacity || 0}`,
      is_critical: formValues.status === "CRITICAL FILL",
    };

    try {
      if (isEditing && selectedResource) {
        const res = await apiFetch(
          `/api/map/resources/${selectedResource.id}`,
          {
            method: "PUT",
          ...
          [activeTab]: prev[activeTab].map((item) =>
            item.id === selectedResource.id
              ? {
                  ...
                  color: normalizeStatusColor(formValues.status),
                }
              : item,
          ),
        }));
        ...
        const newResource = {
          ...
          location: data.resource.type_label || "Unknown location",
          capacity: parseInt(formValues.capacity),
          maxCapacity: parseInt(formValues.maxCapacity),
          status: data.resource.status,
          channel: data.resource.distance_label,
          color: data.resource.status_color_class,
          category: activeTab,
        };
        setResources((prev) => ({
          ...prev,
  ...
  const handleEditResource = (resource) => {
    ...
    setFormValues({
      ...
      maxCapacity: resource.maxCapacity.toString(),
      status: resource.status,
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await apiFetch(`/api/map/resources/${selectedResource.id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      ...
      setResources((prev) => ({
        ...
        [activeTab]: prev[activeTab].filter(
          (res) => res.id !== selectedResource.id,
        ),
      }));
                        ...
                        >
                          <td className="px-6 py-5">
                            <div className="flex items-center gap-4">
                              <div
                                className={`h-10 w-1.5 rounded-full ${res.color}`}
                              />
                              <div>
                          ...
                          <td className="px-6 py-5 text-xs font-semibold text-slate-400 uppercase tracking-widest">
                            {res.location}
                          </td>
                          <td className="px-6 py-5">
                            <div className="space-y-2">
                              <p className="text-[10px] uppercase font-black">
                                {res.capacity}/{res.maxCapacity}
                              </p>
                              <div className="h-1.5 w-32 rounded-full bg-white/5 overflow-hidden">
                                <div
                                  className={`${res.color} h-full transition-all duration-1000`}
                                  style={{
                                    width: `${(res.capacity / res.maxCapacity) * 100}%`,
                                  }}
                                />
                              </div>
                            </div>
                          ...
                          <td className="px-6 py-5">
                            <span
                              className={`inline-flex rounded-lg px-3 py-1 text-[9px] font-black uppercase tracking-widest border ${res.status === "CRITICAL FILL" ? "bg-red-900/20 text-red-400 border-red-500/20" : "bg-sky-900/20 text-sky-400 border-sky-500/20"}`}
                            >
                              {res.status}
                            </span>
                          </td>
                            ...
                            {openMenuId === res.id && (
                              <div className="absolute right-6 top-12 bg-slate-950 border border-white/10 rounded-2xl shadow-2xl z-50 min-w-40 overflow-hidden">
                                ...
                                <button
                                  ...
                                  className="w-full flex items-center gap-3 px-4 py-3 text-xs text-red-400 hover:bg-white/5"
                                >
                ...
                <div className="rounded-[40px] border border-slate-700 bg-linear-to-br from-slate-900 to-slate-950 p-10 shadow-xl">
                  ...
                  <div className="space-y-6">
                    <p className="text-6xl font-light text-white tracking-tighter">
                      84.2<span className="text-xl text-sky-500 ml-1">%</span>
                    </p>
                    <StatBar
                      label="Shelter Load"
                      value={91}
      ...
      {showDetailModal && selectedResource && (
        ...
        >
          ...
          >
            ...
            <div
              className={`h-2 w-full ${selectedResource.color} shadow-[0_0_20px_rgba(255,255,255,0.2)]`}
            />
            ...
            <div className="p-10 space-y-10">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2 font-mono uppercase tracking-[0.4em]">
                    {selectedResource.id}
                  </p>
                </div>
                <button
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-900/95 border border-slate-700 rounded-4xl p-6 group hover:border-sky-500/30 transition-all duration-500">
                  ...
                  <p className="text-lg font-bold text-slate-200 uppercase leading-tight">
                ...
                <div className="bg-slate-900/95 border border-slate-700 rounded-4xl p-6 group hover:border-sky-500/30 transition-all duration-500">
                  <div className="flex items-center gap-3 text-slate-600 mb-4">
                    <Radio
                      size={18}
                      className="group-hover:text-sky-400 animate-pulse"
                    />
                    <span className="text-[10px] font-black uppercase tracking-widest">
                      Comm Channel
                    </span>
                ...
                <div className="bg-slate-900/95 border border-slate-700 rounded-4xl p-6 group hover:border-sky-500/30 transition-all duration-500">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                      <div
                        className={`${selectedResource.color} h-full transition-all duration-1000`}
                        style={{
                          width: `${(selectedResource.capacity / selectedResource.maxCapacity) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
                ...
                <div className="bg-slate-900/95 border border-slate-700 rounded-4xl p-6 group hover:border-sky-500/30 transition-all duration-500 flex flex-col justify-between">
                  ...
                  <div
                    className={`px-4 py-2 rounded-2xl border text-[11px] font-black uppercase tracking-[0.2em] text-center ${selectedResource.status === "CRITICAL FILL" ? "bg-red-500/10 border-red-500/30 text-red-400" : "bg-sky-500/10 border-sky-500/30 text-sky-400"}`}
                  >
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className="flex-1 py-5 rounded-3xl bg-sky-500 text-slate-950 text-[10px] font-black uppercase tracking-[0.3em] shadow-2xl shadow-sky-500/20 hover:bg-sky-400 transition-all"
                >
        ...
        >
          ...
          >
            ...
            >
              ...
              <div className="sm:col-span-2">
                <label className="text-[10px] font-black uppercase text-slate-600 tracking-widest ml-2">
                  Condition Status
                </label>
                <select
                  name="status"
              ...
              <div className="sm:col-span-2 flex justify-end gap-4 mt-4">
                <button
                  type="button"
                  onClick={closeAddModal}
                  className="text-[10px] font-black uppercase text-slate-500 px-6 tracking-widest"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="bg-sky-500 text-slate-950 px-10 py-4 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl"
                >
                  Execute Entry
                </button>
              </div>
      ...
      {showDeleteConfirm && (
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-slate-500 mb-10 leading-relaxed uppercase tracking-widest text-[10px]">
              Irreversible Purge:{" "}
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className="flex-1 py-4 rounded-full bg-red-600 text-white text-[10px] font-black uppercase tracking-widest shadow-xl shadow-red-600/20"
              >
                Purge Data
              </button>
            </div>
...
const StatBar = ({ label, value, color }) => (
  <div className="space-y-3">
    ...
    <div className="h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
      <div
        className={`${color} h-full transition-all duration-1000 shadow-[0_0_10px_rgba(255,255,255,0.1)]`}
        style={{ width: `${value}%` }}
      />
    </div>
  </div>
